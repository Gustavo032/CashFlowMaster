...
class TransactionMapper:
    ...
    def map_transaction(self, transaction: Transaction) -> Transaction:
        ...
        try:
            ...
            if self._should_ignore_transaction(transaction):
                ...
                transaction.historico_contabil = (
                    "Transação ignorada - Saldo informativo"
                )
                return transaction
            ...
            # No mapping found
            logger.info(
                f"No mapping found for transaction: {transaction.descricao_original}"
            )
            return transaction
    ...
    def matches_custom_rule(self, transaction: Transaction, rule: CustomRule) -> bool:
        ...
        try:
            ...
            if rule.tipo_movimentacao_regra != "ambos":
                if (
                    rule.tipo_movimentacao_regra == "entrada"
                    and transaction.tipo_movimentacao != "Crédito"
                ):
                    return False
                if (
                    rule.tipo_movimentacao_regra == "saida"
                    and transaction.tipo_movimentacao != "Débito"
                ):
                    return False
    ...
    def _check_standard_mappings(
        self, transaction: Transaction
    ) -> Optional[Tuple[AccountingMapping, dict, int]]:
        ...
        try:
            ...
            for mapping in mappings:
                ...
                if (
                    mapping.tipo_transacao == "entrada"
                    and transaction.tipo_movimentacao != "Crédito"
                ):
                    continue
                if (
                    mapping.tipo_transacao == "saida"
                    and transaction.tipo_movimentacao != "Débito"
                ):
                    continue
                ...
                if sub_mapping_match:
                    ...
                    if score > best_score:
                        best_match = (
                            mapping,
                            {"type": "sub_mapping", "data": sub_mapping_match},
                            score,
                        )
                        best_score = score
                    continue
                ...
                if main_mapping_match:
                    ...
                    if score > best_score:
                        best_match = (
                            mapping,
                            {"type": "main_mapping", "data": main_mapping_match},
                            score,
                        )
                        best_score = score
    ...
    def _has_exceptions(
        self, transaction: Transaction, mapping: AccountingMapping
    ) -> bool:
    ...
    def _apply_custom_mapping(
        self, transaction: Transaction, rule: CustomRule
    ) -> Transaction:
    ...
    def _should_ignore_transaction(self, transaction: Transaction) -> bool:
        """Check if transaction should be ignored (like SALDO DO DIA)"""
        try:
            ignore_keywords = ["saldo do dia", "saldo anterior", "saldo atual"]
    ...
    def _apply_standard_mapping(
        ...
        mapping_data: Tuple[AccountingMapping, dict, int],
    ) -> Transaction:
        ...
        try:
            ...
            if match_data["type"] == "sub_mapping":
                ...
                transaction.rotulo_contabil = sub_mapping.get(
                    "rotulo_contabil", mapping.rotulo_contabil
                )
                transaction.conta_debito = sub_mapping.get(
                    "conta_debito", mapping.conta_debito
                )
                transaction.conta_credito = sub_mapping.get(
                    "conta_credito", mapping.conta_credito
                )
                transaction.historico_contabil = sub_mapping.get(
                    "historico_contabil_padrao", mapping.historico_contabil_padrao
                )
            else:
